...
function taskThree() {
  ...
  if (day === 0 || day === 6) alert("Woohooo it's weekend");
  else alert("FML");
}
...
function multiplier(valueOne, valueTwo) {
  ...
  else return "Produkten är: [" + valueOne * valueTwo + "]";
}

function taskFour() {
  let result = multiplier(5, 5);
  alert(result);
}
...
function taskFive() {
  for (let i = 0; i < fruits.length; i++) {
    ...
    document.getElementById(
      "answer-five"
    ).innerHTML = `<b>Ätligt:</b> ${eatable.toString()} <br/> <b>Skräp:</b> ${trash.toString()}`;
  }
}

// Ett annat sätt att lösa uppgift 5:
/*
fruits.forEach((fruit) => {
    if (fruit === "apelsin" || fruit === "päron") trash.push(fruit);
...
function taskSix() {
  ...
  document.getElementById(
    "answer-six"
  ).innerHTML = `<b>Över 30:</b> ${overThirty.toString()}
	<b>Under 30:</b> ${underThirty.toString()}
	<b>Gift:</b> ${married.toString()}
	<b>Ogift:</b> ${notMarried.toString()}`;
}
...
function taskSeven() {
  let newLine = addMeSomeLineBreaks
    .split(",")
    .reduce((accumulator, currentval) => {
      return `${accumulator}${", <br/>"}${currentval}`;
    });
  document.getElementById("answer-seven").innerHTML = newLine;
}

// Ett annat sätt att lösa uppgift 7:
/* 
  function taskSeven() {
    let newLine = addMeSomeLineBreaks.split(",").join(", <br/>");
    document.getElementById("answer-seven").innerHTML = newLine;
  }
*/
...
function calculator(valueOne, valueTwo, operator) {
  if (isNaN(valueOne) || isNaN(valueTwo)) {
    ...
    if (operator === "add") return parseInt(valueOne) + parseInt(valueTwo);
...
function taskTen() {
  ...
  let result = calculator(10, 2, "add");
  alert(result);
}
